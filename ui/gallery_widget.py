...
class ThumbnailCard(QFrame):
    ...
    def _load_pixmap(label: QLabel, path: str):
        ...
        label.setText("?")
        label.setStyleSheet(
            f"background:{COLORS['paper_2']};color:{COLORS['ink_3']};font-size:22px;"
        )
...
class ImageViewerDialog(QDialog):
    def __init__(self, image_path: str, parent=None):
        ...
        scroll.setStyleSheet(
            f"border:1px solid {COLORS['paper_3']};border-radius:4px;"
        )
...
class GalleryWidget(QWidget):
    ...
    def _build_ui(self):
        ...
        self._unsaved_msg.setStyleSheet(
            f"color:{COLORS['ink_3']};font-style:italic;font-size:13px;"
        )
    ...
    def _view_full_size(self):
        ...
        if img:
            dlg = ImageViewerDialog(
                os.path.join(IMAGES_DIR, img["path"]), self
            )
            dlg.exec()
